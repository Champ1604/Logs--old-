	...
	message: function (message, room) {
		...
		switch (spl[1]) {
			...
			case 'J': case 'j':
				...
				//if (toId(user.id)=="mmm" && room.id=="battledome") this.say(room, "Taistelu is a nerd"); // Removed via staff request
				//if (toId(user.id)=="taistelu" && room.id=="battledome") this.say(room, "Long live the Princess!");
				//vulc's joinphrase was here but they removed it like the nerd they are
				//if (toId(user.id)=="keskas" && room.id=="battledome") this.say(room, "The Chomp is in!"); // This is for the Game Mode competition they won
				//if (toId(user.id)=="zestoflife" && room.id=="battledome") this.say(room, "Life is __DANK__"); //1v1 tour prize
